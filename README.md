...
- 브라우저의 오늘보다 미래인 원본 거래는 전체 업로드 건수에는 남기되 과거 실적·월별 집계·카테고리·반복 거래·수입 추세·최근 잔액·전망 기준월에서는 제외합니다. 날짜·금액·방향이 유효하고 현재 3개월 전망 범위 안에 있으면 Forecast 예정 거래로 자동 반영하며 사용자가 개별 거래를 제외하거나 다시 포함할 수 있습니다. 기간 밖 거래는 목록에 보존하지만 현재 Forecast에는 반영하지 않습니다.
