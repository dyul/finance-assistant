...
- 분류: `BUG / COMPATIBILITY / TRUST`
- 심각도: `P1`
- 내용: 최신순으로 정렬된 실제 은행 거래내역의 동일 날짜 거래 시각이 날짜 정규화 중 사라져, 더 이른 거래의 잔액이 최근 잔액으로 선택됨
- 개인정보·은행명·실제 거래 시각·금액·잔액·파일명: 기록하지 않음
- 반영 상태: Day 45.1에서 날짜별 유효 잔액 후보 전체를 기준으로 명시적 시각 또는 원본 행 순서를 일관되게 적용하는 total-order chronology를 구현하고, 합성 XLS/XLSX/CSV 및 수동·자동 매핑으로 회귀 검증, 실제 원본 재검증 필요

- 분류: `FEATURE REQUEST / VALUE / UX`
- 내용: 월 중간에 사업을 시작한 경우처럼 달력 월과 다른 임의 운영 기간의 수입·지출·순현금흐름을 하나의 구간으로 확인하고 싶음
- 개인정보·실제 기간·금액·파일명: 기록하지 않음
- 반영 상태: Day 46에서 양 끝 날짜를 포함하는 사용자 지정 과거 기간 통합 요약과 월·분기·연도 상세를 구현하고, Forecast는 전체 분석 기준으로 유지

- 분류: `VALUE / UX / CLASSIFICATION`
- 내용: 자영업자 관점의 기존 분류는 비교적 자연스럽지만 개인 사용자 관점에서는 적금 같은 금융성 거래가 모두 기타로 묶여 의미 구분이 어렵다는 의견
- 개인정보·실제 금액·거래처·계좌·파일명: 기록하지 않음
- 반영 상태: Day 47에서 현금 방향과 금융 거래 성격을 분리하고 저축/적금·투자·대출/상환·내부이체 표시를 추가하되, 단일 파일 현금흐름과 Forecast에는 실제 입출금을 계속 포함
...
- `BUG / COMPATIBILITY / TRUST`: 동일 날짜의 명시적 거래 시각이 잔액 chronology에 반영되지 않아 최근 잔액이 잘못 선택됨 — P1, Day 45.1 합성 회귀 통과 및 실제 원본 재검증 필요.
