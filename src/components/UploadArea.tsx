...
export default function UploadArea() {
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setColumnMappings(mapColumns(columnNames));
  ...
  function getConfidenceLabel(confidence: ColumnMapping["confidence"]) {
    if (confidence === "high") {
      return "높음";
    }
  ...
  function getConfidenceStyle(confidence: ColumnMapping["confidence"]) {
    if (confidence === "high") {
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <label className="flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-slate-300 px-6 py-10 text-center transition hover:border-blue-400 hover:bg-blue-50">
        <span className="font-medium text-slate-700">엑셀 파일 선택</span>
          ...
          <div className="overflow-hidden rounded-lg border border-slate-200">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  <th className="px-4 py-3 font-medium">원본 컬럼</th>
                  <th className="px-4 py-3 font-medium">인식 결과</th>
